...
function Todos({isAuthenticated, setIsAuthenticated}) {
	...
	useEffect(() => {
		const loadData = async () => {
			let response = null;
			try {
				let url = `http://107.22.147.17:3001/api/todo/${pageNumber - 1}/${pageSize}`;

				if(filter === 'Completed'){
					url = `http://107.22.147.17:3001/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=true`;
				} else if(filter === 'Not Completed'){
					url = `http://107.22.147.17:3001/api/todo/${pageNumber - 1}/${pageSize}?isCompleted=false`;
				}
	...
	const markCompleted = async (id) => {
		try {
      await axios.put(`http://107.22.147.17:3001/api/todo/${id}/markcomplete`, {}, {
				headers: {
	...
	const deleteTodo = async (id) => {
		try {
      await axios.delete(`http://107.22.147.17:3001/api/todo/${id}`, {
				headers: {
					'Authorization': `Bearer ${sessionStorage.getItem('token')}`
				}
			});
